...
// ================================================== QUESTÃO 1  ================================================== //
 ...
 $letrasFoo     = array('r', 't', 'c', 'd', 'b');
 $totalProp     = 0;
 $explodeTxtB   = explode(' ', $textoB);

 foreach($explodeTxtB as $key => $palavra)
 {
    if( (strlen($palavra) == 5 ) && (!in_array($palavra[4], $letrasFoo)) && (strpos($palavra, 'l') === false))
    {
        $totalProp++;
    }
 }

 echo "Existem $totalProp preposições no Texto B <br>";
...
// ================================================== QUESTÃO 2  ================================================== //
 ...
 foreach($explodeTxtB as $key => $palavra)
 {
    if( (!in_array($palavra[(strlen($palavra)-1)], $letrasFoo)) && (strlen($palavra) >= 8))
    {
        if(!in_array($palavra[0], $letrasFoo))
        {
            $verboPrimPessoa++;
        }
       $totalVerbo++;
...
echo "Há $totalVerbo verbos no Texto B <br>";
echo "Há $verboPrimPessoa verbos em primeira pessoa no Texto B <br><br>";

// ================================================== fim - QUESTÃO 2  ================================================== //
 
// ================================================== QUESTÃO 3  ================================================== //

$arrayTextB = array_unique($explodeTxtB);
usort($arrayTextB, "ordernarBooglan");
$vocabularioFinal = implode(' ', $arrayTextB);

echo "<br><strong>A lista de vocabulário do Texto B seria:</strong><br> $vocabularioFinal";
...
foreach($explodeTxtB as $key => $word)
{
    $i = 1;
    $somaNumBon = 0;    
    while($i <= strlen($word))
    {
        $pos = strpos($alfabeto, $word[$i-1]);
        $somaNumBon = bcadd($somaNumBon, fatoracao($pos, $i)); 
        $i++;
    }

    if(($somaNumBon >= 422224) && ($somaNumBon % 3 ==  0) ) 
    {
        $numBonito++;
    }
}
echo  "<br><br>No Texto B, há $numBonito números bonitos distintos (atenção!)";
